...
for (let i = 0; i < Part.COUNT; ++i) {
  ...
  floorTextures[i] = new THREE.ImageUtils.loadTexture('images/dancefloor.jpg');
...
var leftEyePosition = {
	type: 'v3',
	value: new THREE.Vector3(0.15, 2.45, -0.65)
}
...
var headRotationSpeed = 0.002;

var armRotationAngle = {
	type: 'f',
	value: 0.0
}

var armRotationSpeed = 0.002;

var leftLegRotationAngle = {
	type: 'f',
	value: 0.0
}
var rightLegRotationAngle = {
	type: 'f',
	value: 0.0
}
...
// (C) NODDING ARMADILLO
var discoMaterial = new THREE.ShaderMaterial({
	uniforms: {
		discoPosition: discoPosition,
		discoRotationAngle: discoRotationAngle
	}
})
...
new THREE.SourceLoader().load(discoShaderFiles, function(shaders) {
  discoMaterial.vertexShader = shaders['glsl//discoball.vs.glsl']
  discoMaterial.fragmentShader = shaders['glsl//discoball.fs.glsl']
...
var noddingArmadilloMaterial = new THREE.ShaderMaterial({
  uniforms: {
	...
	armRotationAngle: armRotationAngle,
	leftLegRotationAngle: leftLegRotationAngle,
	rightLegRotationAngle: rightLegRotationAngle
  }
})
...
var leftLaserMaterial = new THREE.ShaderMaterial({
	uniforms: {
		laserPosition: leftLaserPosition,
		lightPosition: lightPosition
	}
});
var rightLaserMaterial = new THREE.ShaderMaterial({
	uniforms: {
		laserPosition: rightLaserPosition,
		lightPosition: lightPosition
	}
});
...
// create discoball
var discoGeometry = new THREE.SphereGeometry(1, 32, 32);
var discoball = new THREE.Mesh(discoGeometry, discoMaterial);
scenes[Part.DEFORM].add(discoball);
  ...
  */
	...
	// Arm spinning speed
	  ...
	  if (keyboard.pressed("E"))
		  // tilt left leg back
		leftLegRotationAngle.value -= 0.1;
	  else if (keyboard.pressed("D"))
		  // tilt left leg forward
		leftLegRotationAngle.value += 0.1;
	  
	  if (keyboard.pressed("R"))
		  // tilt right leg back
		rightLegRotationAngle.value -= 0.1;
	  else if (keyboard.pressed("F"))
		  // tilt right leg forward
		rightLegRotationAngle.value += 0.1;
  }
	...
	discoMaterial.needsUpdate = true;
	
	armRotationAngle.value = (Date.now() * armRotationSpeed)%100;
	rotationAngle.value = (Date.now()* headRotationSpeed)%100;
	discoRotationAngle.value = (Date.now()* 0.005)%100;
